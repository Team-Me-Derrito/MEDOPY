...
"""
getAccount()
    Gets the account with the given account_id and token.
    If the account doesn't exist returns success=Fasle and gives an error.
    @param account_id - account's id
    @param token - account's current session token
"""
def getAccount(account_id, token):
    accounts = Account.objects.filter(pk=account_id, token=token)

    if len(accounts) != 1:
        return {"success": False, "message": "No account with those credentials exists."}
    else:
...
getInterestEventsByAccount()
    ...
    @param account_token
"""
def getInterestEventsByAccount(account):
